...
class BrandAdapter(val items:MutableList<BrandModel>) :
    ...
    override fun onBindViewHolder(holder: BrandAdapter.Viewholder, @SuppressLint("RecyclerView") position:Int) {
        ...
        if(selectedPosition==position){
           holder.binding.picBrand.setBackgroundResource(0)
            holder.binding.mainLayout.setBackgroundResource(R.drawable.purple_button_bg)
            ImageViewCompat.setImageTintList(
                holder.binding.picBrand,
                ColorStateList
                    .valueOf(context.getColor(R.color.white)))
            holder.binding.title.visibility=View.VISIBLE
        }
        else {
            holder.binding.picBrand.setBackgroundResource(R.drawable.grey_bg)
            holder.binding.mainLayout.setBackgroundResource(0)
            ImageViewCompat.setImageTintList(
                holder.binding.picBrand,
                ColorStateList
                    .valueOf(context.getColor(R.color.black)))
